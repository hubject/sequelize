...
var QueryGenerator = {
  ...
  /**
   * Returns select attributes of main table
   *
   ...
   * @return {Array}
   */
  getSelectAttributes: function (mainAttributes, mainTableAs, options) {

    var self = this;

    return mainAttributes.map(function (attr) {
      var addTable = true;
  ...
  /**
   * Returns select attributes of joined tables
   *
   * @param include
   * @param as
   * @return {Array}
   */
  getSelectIncludeAttributes: function (include, as) {

    var self = this;

    return include.attributes.map(function (attr) {
      var attrAs = attr,
        verbatim = false;
      ...
      var prefix;
      if(include.alias) {
        prefix = self.quoteIdentifier(include.alias) + '.' + self.quoteIdentifier(attr);
      } else if (verbatim === true) {
        prefix = attr;
      } else {
  ...
  selectQuery: function(tableName, options, model) {
    ...
    // Escape attributes
    mainAttributes = mainAttributes && self.getSelectAttributes(mainAttributes, mainTableAs, options);
    ...
    if (options.include) {
      var generateJoinQueries = function(include, parentTable) {
        ...
        if (options.includeIgnoreAttributes !== false) {
          attributes = self.getSelectIncludeAttributes(include, as);
  ...
  addQuerySessionVariables: function (sql, variables) {

    throw new Error('Session variables not implemented for dialect "' + this.dialect + '".');
  },

  removeSessionVariableResults: function (sql, variables) {
