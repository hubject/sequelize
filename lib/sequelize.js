...
Sequelize.prototype.ATTR_QUERY_PLACEHOLDER = '<SQ_MODEL_ATTRS>';
...
Sequelize.prototype.query = function(sql, options) {
  ...
  var self = this,
    attrPlaceholderRegex = new RegExp(this.ATTR_QUERY_PLACEHOLDER);

  options = _.assign({}, this.options.query, options);

  // BEGIN: ATTR_QUERY_PLACEHOLDER
  // -------------------------------------

  // If sql contains ATTR_QUERY_PLACEHOLDER, generate
  // select attributes from options
  if (attrPlaceholderRegex.test(sql)) {

    var attributes;

    // prepare main table attributes
    if(options.attributes) {
      attributes = options.attributes
    } else {
      var tableAttributes = options.model.tableAttributes;
      attributes = [];

      for(var attrKey in tableAttributes) {
        if(tableAttributes.hasOwnProperty(attrKey)) {

          attributes.push(tableAttributes[attrKey].field);
        }
      }
    }

    // If options contains include, set hasJoin true
    options.hasJoin = !!options.include;

    var queryAttributes = this.dialect.QueryGenerator.getSelectAttributes(
      attributes, options.alias, options);

    if (options.hasJoin) {

      // process joined table attributes
      Model.$validateIncludedElements(options);
      options.include.forEach(function (include) {
        setSelectIncludeAttributes(queryAttributes, include)
      })
    }

    sql = sql.replace(attrPlaceholderRegex, queryAttributes.join(','));
  }

  /**
   * Sets attributes of all included models 	recursively
   *
   * @param queryAttributes
   * @param include
   * @param parentAs
   */
  function setSelectIncludeAttributes(queryAttributes, include, parentAs) {

    // alias is required; by sequelize implicitly added
    // includes, will have no alias property
    if(include.alias) {

      var _as = parentAs ? parentAs + '.' + include.as : include.as;
      queryAttributes.push.apply(queryAttributes,
        self.dialect.QueryGenerator.getSelectIncludeAttributes(include, _as));

      if (include.include) {
        include.include.forEach(function (_include) {
          setSelectIncludeAttributes(queryAttributes, _include, _as);
        });
      }
    }
  }

  // END: ATTR_QUERY_PLACEHOLDER
  // -------------------------------------
  ...
  if(options.sessionVariables) {
    sql = this.dialect.QueryGenerator.addQuerySessionVariables(sql, options.sessionVariables);
  }
